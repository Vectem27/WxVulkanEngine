...
void VulkanBuffer::Update(const void *data, VkDeviceSize offset, size_t size)
{
    if (bufferMemory == VK_NULL_HANDLE)
    {
        Log(Warning, "Vulkan", "Failed to update buffer, bufferMemory is null");
        return;
    }

    if (!data) 
    {
        Log(Warning, "Vulkan", "Failed to update buffer, data is null");
        return;
    }

    if (offset + size > bufferSize) 
    {
        Log(Warning, "Vulkan", "Failed to update buffer : offset + size exceeds buffer size");
        return;
    }
    ...
    {
        Log(Warning, "Vulkan", "Failed to update buffer memory");
    }
}
